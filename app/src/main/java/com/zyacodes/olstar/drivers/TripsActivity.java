...
public class TripsActivity extends AppCompatActivity {
    ...
    private void onClientNoShow(TripModel trip) {
        ...
        schedulesRef.child(trip.getTripId())
                ...
                .addOnSuccessListener(aVoid -> {
                    ...
                    Toast.makeText(TripsActivity.this, "✅ Trip #" + trip.getTripNumber() + " marked as Client No Show", Toast.LENGTH_LONG).show();
                })
                .addOnFailureListener(e -> Toast.makeText(TripsActivity.this, "Failed to mark no show: " + e.getMessage(), Toast.LENGTH_SHORT).show());
    }

    private void displayCurrentDate() {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("EEEE, MMM d, yyyy", Locale.US);
    ...
    private void loadUserFromPrefs() {
        driverPhone = getSharedPreferences("login", MODE_PRIVATE).getString("phone", null);
        if (driverPhone == null) {
    ...
    private void setupFirebase() {
        schedulesRef = FirebaseDatabase.getInstance("https://olstar-5e642-default-rtdb.asia-southeast1.firebasedatabase.app/").getReference("schedules");
    }

    private void loadRFIDDataAndTrips() {
        DatabaseReference rfidRef = FirebaseDatabase.getInstance("https://olstar-5e642-default-rtdb.asia-southeast1.firebasedatabase.app/").getReference("rfidCards");
            ...
            public void onDataChange(@NonNull DataSnapshot rfidSnapshot) {
                ...
                for (DataSnapshot cardSnapshot : rfidSnapshot.getChildren()) {
                    ...
                    if (plateNumber != null && !plateNumber.isEmpty()) {
                        ...
                        rfidCache.put(plateNumber, new RFIDCardData(balance != null ? balance : 0.0, lastUpdated != null ? lastUpdated : 0L));
                    }
                }
                loadTodayTrips();
            }
    ...
    private void loadTodayTrips() {
        ...
        DateTimeFormatter dateDisplayFormatter = DateTimeFormatter.ofPattern("EEEE, MMMM d, yyyy", Locale.US);
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot sched : snapshot.getChildren()) {
                    ...
                    TripModel trip = new TripModel(
                            ...
                            rfidBalance,
                            rfidLastUpdated,
                            sched.child("company").getValue(String.class),
                            sched.child("pax").getValue(String.class)
                    );

                    boolean isIncomplete = status != null && !"Completed".equalsIgnoreCase(status) && !"Cancelled".equalsIgnoreCase(status) && !"No Show".equalsIgnoreCase(status);

                    if (tripDate.equals(yesterday)) {
                        allYesterdayTrips.add(trip);
                        if (isIncomplete) activeYesterdayTrips.add(trip);
                    } else if (tripDate.equals(today)) {
                        allTodayTrips.add(trip);
                        if (isIncomplete) activeTodayTrips.add(trip);
                    } else if (tripDate.equals(tomorrow)) {
                        allTomorrowTrips.add(trip);
                        if (isIncomplete) activeTomorrowTrips.add(trip);
                    }
                }
                ...
                int tripNumber = 1;
                for (TripModel trip : allYesterdayTrips) trip.setTripNumber(tripNumber++);
                tripNumber = 1;
                for (TripModel trip : allTodayTrips) trip.setTripNumber(tripNumber++);
                tripNumber = 1;
                for (TripModel trip : allTomorrowTrips) trip.setTripNumber(tripNumber++);
    ...
    private void initCameraLauncher() {
        cameraLauncher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
            if (result.getResultCode() == RESULT_OK && currentTripForPhoto != null && currentPhotoUri != null) {
    ...
    private void annotateAndSavePhoto() {
        ...
        Bitmap annotatedBitmap = burnTextOnImage(currentPhotoUri, currentTripForPhoto.getDriverName(), currentTripForPhoto.getClientName(), dateTime, currentCoordinates);

        if (annotatedBitmap != null) {
            try {
                File annotatedFile = new File(getExternalFilesDir(Environment.DIRECTORY_PICTURES), "OLStar/ANNOTATED_" + System.currentTimeMillis() + ".jpg");
    ...
    private Bitmap burnTextOnImage(Uri photoUri, String driverName, String clientName, String dateTime, String coordinates) {
        try {
            ...
            int padding = 40;
            float x, y;
            List<String> lines = Arrays.asList("Driver: " + driverName, "Client: " + clientName, "Date: " + dateTime, "Coordinates: " + coordinates);
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == REQUEST_CAMERA_PERMISSION) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED && currentTripForPhoto != null && currentPhotoType != null) {
        ...
        MediaManager.get().upload(uri).unsigned("images").callback(new UploadCallback() {
            @Override public void onSuccess(String requestId, Map resultData) {
                ...
                schedulesRef.child(trip.getTripId()).child("PhotoUrl").child(photoType).setValue(url).addOnSuccessListener(aVoid -> {
                    ...
                    schedulesRef.child(trip.getTripId()).child("status").setValue(newStatus).addOnSuccessListener(v -> {
                        trip.setStatus(newStatus);
                        for (int i = 0; i < combinedList.size(); i++) {
                            Object item = combinedList.get(i);
                            if (item instanceof TripModel && ((TripModel) item).getTripId().equals(trip.getTripId())) {
                                adapter.notifyItemChanged(i);
                                break;
                            }
                        }
                        Toast.makeText(TripsActivity.this, "Photo uploaded & status updated to " + newStatus, Toast.LENGTH_SHORT).show();
                    });
                });
            }
            @Override public void onError(String requestId, ErrorInfo error) { Toast.makeText(TripsActivity.this, "Upload failed!", Toast.LENGTH_SHORT).show(); }
